...
普通改动验收只检查实际差异、相关验证和风险边界，不重新做完整探索；证据矛盾、验证失败或高风险信号出现时才扩大复核。主控按风险独立验收，不把叶子自述或退出码当作证据。
...
使用可用工具的实际参数；只需任务上下文时使用有界/无历史委派，避免无关上下文传给子 Agent。五个角色均为叶子，不得再次委派或自行刷新策略。主 Agent 等待时只处理不同且不重叠的工作；没有剩余独立工作则等待，不重复叶子负责范围，也不为等待启动 Agent。主 Agent 等待并检查真实产物、边界和相关测试，必要时把普通失败返给原叶子修复；不要用叶子自述或退出码替代验收。主 Agent 仍可接回必要工作，但要说明是公共接口/重大方案、重复失败或能力/权限限制等原因。叶子交付五点精简结论：完成、文件位置、验证证据、未决问题、待决事项；详细日志放在指定本地路径按需读取，失败和风险不得省略。若工具提供关闭接口，在完成后关闭；若只有等待、状态与中断接口，确认子 Agent 已完成且停止写入即可，名额由宿主管理，不虚构关闭调用或名额释放证据。保留可观测的运行证据。
